...
def detect_remove_OA(original_EEG,dispIMG):
    ...
    if ocularArtifacts.size > 0:
        wavelet = 'haar'
        coeffs = (wavedec(ocularArtifacts,wavelet,level=5))
...
tech_1 = detect_remove_OA(rawEEG[:,0],False)
tech_2 = detect_remove_OA(rawEEG[:,1],False)
tech_3 = detect_remove_OA(rawEEG[:,2],False)
plt.plot(time,tech_1,color='blue',label='tech_1')
plt.show()
plt.plot(time,tech_2,color='red',label='tech_2')
plt.show()
plt.plot(time,tech_3,color='green',label='tech_3')
plt.show()

new_raw = np.vstack((tech_1,tech_2,tech_3)).T
spectogramPlot(new_raw,fs,nfft=cfg.nfft,nOverlap=cfg.noverlap,figsize=(16,6),subTitles=cfg.channelNames,title='Music Therapy Group 11')
